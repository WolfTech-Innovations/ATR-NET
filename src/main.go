package main
import("bytes";"crypto/aes";"crypto/cipher";"crypto/ed25519";"crypto/rand";"crypto/sha256";"encoding/base64";_"encoding/binary";"encoding/json";"fmt";"io";"net";"net/http";"os";_"path/filepath";_"sort";"strconv";"strings";"sync";"time";"bufio")
...
var(NETID="ATR-NET-V1";MAXHOPS=9;CHUNKS=7;PKTSIZE=65536;BLKSIZE=1048576;NODEPORT int=7777;BOOTPORT int=7778;DNSPORT int=7779;WEBPORT int=7780);func main(){var s *bufio.Scanner=bufio.NewScanner(os.Stdin);fmt.Print("Use defaults? (Y/N): ");s.Scan();if strings.ToLower(s.Text())=="y"{net,e:=NewNET();if e!=nil{fmt.Printf("FAILED TO START ATR-NET: %v\n",e);os.Exit(1)};e=net.START();if e!=nil{fmt.Printf("FAILED TO RUN ATR-NET: %v\n",e);os.Exit(1)};fmt.Println("ATR-NET IS LIVE!");fmt.Printf("Bootstrap: :%d\nDNS: :%d\nWeb: :%d\nNodes: :%d\n",BOOTPORT,DNSPORT,WEBPORT,NODEPORT);select{}};fmt.Print("Bootstrap port: ");s.Scan();bp:=s.Text();fmt.Print("DNS port: ");s.Scan();dp:=s.Text();fmt.Print("Web port: ");s.Scan();wp:=s.Text();fmt.Print("Node port: ");s.Scan();np:=s.Text();fmt.Print("Peer IPs (comma-sep): ");s.Scan();pi:=strings.Split(strings.ReplaceAll(s.Text()," ",""),",");if s.Text()==""{pi=[]string{}};for _,p:=range []string{bp,dp,wp,np}{if n,e:=strconv.Atoi(p);e!=nil||n<1||n>65535{fmt.Println("Invalid port");os.Exit(1)};if c,e:=net.Dial("tcp",":"+p);e==nil{c.Close();fmt.Printf("Port %s in use\n",p);os.Exit(1)}};BOOTPORT,_=strconv.Atoi(bp);DNSPORT,_=strconv.Atoi(dp);WEBPORT,_=strconv.Atoi(wp);NODEPORT,_=strconv.Atoi(np);_=pi;net,e:=NewNET();if e!=nil{fmt.Printf("FAILED TO START ATR-NET: %v\n",e);os.Exit(1)};e=net.START();if e!=nil{fmt.Printf("FAILED TO RUN ATR-NET: %v\n",e);os.Exit(1)};fmt.Println("ATR-NET IS LIVE!");fmt.Printf("Bootstrap: :%d\nDNS: :%d\nWeb: :%d\nNodes: :%d\n",BOOTPORT,DNSPORT,WEBPORT,NODEPORT);if len(pi)>0{fmt.Printf("Peers: %s\n",strings.Join(pi,", "))};select{}}
